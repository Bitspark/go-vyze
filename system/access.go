...
func ReadLayerToken(tokenString string) (LayerToken, error) {
	...
	if err == nil && (len(msgBytes) == msgLen+seedLength+signLength || len(msgBytes) == msgLen+seedLength-1+signLength) {
		goto succ
	}
...
succ:
	...
	if len(msgBytes) >= msgLen+seedLength {
		...
		s.Signature = msgBytes[msgLen-1+seedLength:]
	}
