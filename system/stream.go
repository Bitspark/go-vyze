...
func (sub *ResourceStreamList) addValues(id core.ID, val any) error {
	ins := sub.GetValue(id)
	if ins == nil {
		return errors.New("not found")
	}
...
func (sub *ResourceStreamList) removeValues(id core.ID, val any) error {
	ins := sub.GetValue(id)
	if ins == nil {
		return errors.New("not found")
	}
...
func (sub *ResourceStreamList) add(id core.ID) (any, error) {
	sub.mux.Lock()
	defer sub.mux.Unlock()
...
func (sub *ResourceStreamList) remove(id core.ID) error {
	sub.mux.Lock()
	defer sub.mux.Unlock()
	idx, ok := sub.indexes[id]
	if !ok {
...
type SystemStreamClient struct {
	endpoint      string
	subscriptions map[core.ID]*Subscription
	conn          *websocket.Conn
	sendMux       sync.Mutex
}
