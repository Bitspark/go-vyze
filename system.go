...
type CreateObjectRequest struct {
	Abstracts IDSet  `json:"abstracts"`
	Name      string `json:"name"`
...
func (c *SystemClient) GetObject(id ID, options *AccessOptions) (Object, error) {
...
func (c *SystemClient) CreateObject(abstractIDs IDSet, name string, accessName string, options *AccessOptions) (Object, error) {
...
func (c *SystemClient) DeleteObject(id ID, options *AccessOptions) error {
	u := c.buildURL(fmt.Sprintf("object/%s", id.Hex()), nil)
...
func (c *SystemClient) GetData(id ID, formatType FormatType, options *AccessOptions) (any, error) {
...
func (c *SystemClient) SetData(id ID, value any, formatType FormatType, options *AccessOptions) error {
	u := c.buildURL(fmt.Sprintf("object/%s/data", id.Hex()), map[string][]string{"format": {string(formatType)}})
	var err error
	var resp interface{}
	switch formatType {
	...
	case FormatTypeRaw:
		resp, err = c.postRaw(u, value.(Binary), options)
	}
	if err != nil {
		return err
	}
...
func (c *SystemClient) DeleteData(id ID, options *AccessOptions) error {
...
func (c *SystemClient) GetName(id ID, options *AccessOptions) (string, error) {
...
func (c *SystemClient) SetName(id ID, name string, options *AccessOptions) error {
	u := c.buildURL(fmt.Sprintf("object/%s/name", id.Hex()), nil)
	err := c.postJSON(u, name, nil, options)
	return err
}

func (c *SystemClient) GetAbstracts(id ID, self bool, direct bool, indirect bool, options *AccessOptions) (IDSet, error) {
...
func (c *SystemClient) GetSpecials(id ID, self bool, direct bool, indirect bool, options *AccessOptions) (IDSet, error) {
	var selfStr, directStr, indirectStr string
	if self {
		selfStr = "1"
	}
...
func (c *SystemClient) CreateRelation(originID ID, targetID ID, abstractIDs IDSet, name string, accessName string, options *AccessOptions) (Object, error) {
...
func (c *SystemClient) CreateKeyedRelation(originID ID, targetID ID, abstractIDs IDSet, name string, key any, keyFormat FormatType, accessName string, options *AccessOptions) (Object, error) {
	u := c.buildURL(fmt.Sprintf("relation"), nil)
	req := CreateRelationRequest{
		Name:      name,
		Origin:    originID,
...
func (c *SystemClient) PutResourceInstance(resource *ResourceInstance, value any, accessName string, options *AccessOptions) (ID, error) {
...
func (c *SystemClient) PutResourceSpecials(resource *ResourceSpecials, values any, accessName string, options *AccessOptions) (IDList, error) {
	u := c.buildURL(fmt.Sprintf("resource/put"), nil)
	var resp IDList
...
func (c *SystemClient) getAccessLayerID(accessName string) ID {
...
func (c *SystemClient) getRaw(u string, options *AccessOptions) (Binary, error) {
	respBytes, err := c.request(u, "GET", nil, options)
	if err != nil {
		return nil, err
	}
...
func (c *SystemClient) deleteRaw(u string, options *AccessOptions) (Binary, error) {
	respBytes, err := c.request(u, "DELETE", nil, options)
	if err != nil {
		return nil, err
	}
...
func (c *SystemClient) postRaw(u string, reqBytes Binary, options *AccessOptions) (Binary, error) {
	respBytes, err := c.request(u, "POST", reqBytes, options)
	if err != nil {
		return nil, err
	}
